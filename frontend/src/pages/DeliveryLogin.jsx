  ...
  useEffect(() => {
    const check = async () => {
      try {
        ...
        if (res.ok && data.success && data.user.role === 'delivery') {
    ...
    try {
      ...
      if (res.ok && data.success) {
        if (user?.role !== 'delivery') {
          ...
          toast.error('Access Denied: This portal is for Delivery Associates only.', { duration: 5000 });
          setLoading(false);
          return;
        }
        toast.success(`Welcome back, ${user.name}! 🛵`);
        setTimeout(() => { window.location.href = '/delivery/dashboard'; }, 800);
      } else {
    ...
    <div style={{
      ...
      <div style={{ width: '100%', maxWidth: '420px' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '32px' }}>
          ...
          <h1 style={{ color: 'white', fontSize: '1.8rem', fontWeight: '800', margin: 0 }}>Delivery Partner</h1>
          <p style={{ color: 'rgba(255,255,255,0.5)', marginTop: '6px', fontSize: '0.95rem' }}>OrderEat Fleet Management Portal</p>
        </div>
        ...
        <div style={{
          ...
          <p style={{ color: 'rgba(255,255,255,0.45)', fontSize: '0.9rem', marginBottom: '28px' }}>Enter your credentials to access your deliveries</p>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <button type="submit" disabled={loading} style={{
              ...
              background: loading ? 'rgba(230,126,34,0.5)' : 'linear-gradient(135deg, #f39c12, #e67e22)',
          ...
          <div style={{ textAlign: 'center', marginTop: '24px' }}>
            <span style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.9rem' }}>New delivery partner? </span>
            <Link to="/delivery/register" style={{ color: '#f39c12', fontWeight: '700', textDecoration: 'none', fontSize: '0.9rem' }}>
              Register here →
            </Link>
          </div>
        </div>
