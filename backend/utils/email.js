...
export default class Email {
  ...
  async sendOTP(otp) {
    // We override render logic slightly because send() assumes this.firstName exists
    const html = pug.renderFile(`${__dirname}/../view/otp.pug`, {
      firstName: this.firstName,
      otp: otp,
    });

    const emailData = {
      sender: { name: "OrderEat", email: this.from },
      to: [{ email: this.to, name: this.firstName }],
    ...
    try {
      ...
      if (!response.ok) throw new Error("Failed to send OTP via Brevo API");
    } catch (error) {
      console.error("Brevo API Error:", error.message);
      throw error;
    }
